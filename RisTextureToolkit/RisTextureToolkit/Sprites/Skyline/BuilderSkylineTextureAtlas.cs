...
namespace RisTextureToolkit.Sprites.Skyline
{
    ...
    public class BuilderSkylineTextureAtlas : ABaseBuilderTextureAtlas
    {
        ...
        /// <inheritdoc/>
        public override BuilderTexture AddSprite(RawImage image)
        {
            ...
            if (FindPosition(nodeWidth, nodeHeight, out int x, out int y, out int bestIndex))
            {
                ...
                BuilderTexture texture = new BuilderTexture(image, new Point(x, y), this);
                _sprites.Add(texture);
                return texture;
            }
        ...
        /// <inheritdoc/>
        public override bool TryAddSprite(RawImage image, out BuilderTexture? sprite)
        {
            int nodeWidth = image.Width + Padding * 2;
            int nodeHeight = image.Height + Padding * 2;
            ...
            if (FindPosition(nodeWidth, nodeHeight, out int x, out int y, out int bestIndex))
            {
                ...
                // Create and add the sprite
                sprite = new BuilderTexture(image, new Point(x + Padding, y + Padding), this);
                _sprites.Add(sprite);
                return true;
            }
            sprite = null;
